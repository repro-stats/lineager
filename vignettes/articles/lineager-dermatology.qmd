...
author: "Ndoh Penn"
date: "2026-06-26"
categories: [lineager, CDISC, ADaM, dermatology, data-provenance, exclusion-tracking]
format:
  html:
    toc: true
    toc-depth: 3
...
dataset carries a lineage ID (`.__lid__`) that survives filters, joins, and
...
adlb_raw$AVAL <- mapply(function(trt, vis, scat) {
  ...
  if (vis == 0L) {
    return(round(pmax(0, rnorm(1, base_mean, 8)), 1))
  }
...
`lg_tag()` assigns a unique lineage ID (`.__lid__`) to every row. The
...
```{r join}
adeff <- lg_join(
  adlb, adsl_itt[, c(".__lid__", "USUBJID", "TRT01P", "EASISCAT")],
  by = "USUBJID",
  type = "inner",
...
adeff <- lg_tag(as.data.frame(adeff),
  dataset_id = "ADEFF", domain = "ADLB",
