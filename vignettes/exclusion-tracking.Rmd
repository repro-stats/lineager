...
analysis_excl[, c("id", "reason", "reason_code")]
```
...
| `usubjid` | Subject identifier if present (from USUBJID column) |
| `id` | Row identifier if an `id` column exists |
...
excluded_id <- lg_exclusions()$id[[1L]]
...
```{r trace-included}
included_id <- analysis_set$id[[1L]]
